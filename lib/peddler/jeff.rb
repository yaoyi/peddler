...
module Jeff
  ...
  def connection
    @connection ||= HTTP::Client.new(connection_params).persistent(aws_endpoint)
  end
  ...
  def sign(options)
    ...
    uri = HTTP::URI.parse aws_endpoint

    # Generate signature.
    signature = Signer
      .new(options[:method], uri.host, options[:path] || uri.path, query_string)
  ...
  module ClassMethods
    ...
    def default_user_agent
      "Jeff/2.0 (Language=Ruby; #{`hostname`.chomp})"
    end
  end
end
